...
class TestBaseModel_instantiation(unittest.TestCase):
	...
	def test_two_models_unique_ids(self):
		damo = BaseModel()
		bamo2 = BaseModel()
		self.assertNotEqual(damo.id, bamo2.id)

	def test_two_models_different_created_at(self):
		damo = BaseModel()
		sleep(0.05)
		bamo2 = BaseModel()
		self.assertLess(damo.created_at, bamo2.created_at)

	def test_two_models_different_updated_at(self):
		damo = BaseModel()
		sleep(0.05)
		bamo2 = BaseModel()
		self.assertLess(damo.updated_at, bamo2.updated_at)

	def test_str_representation(self):
		datetime = datetime.now()
		datetime_repo = repr(datetime)
		bamo = BaseModel()
		bamo.id = "123456"
		bamo.created_at = bamo.updated_at = datetime
		damostr = bamo.__str__()
		self.assertIn("[BaseModel] (123456)", damostr)
		self.assertIn("'id': '123456'", damostr)
		self.assertIn("'created_at': " + datetime_repo, damostr)
		self.assertIn("'updated_at': " + datetime_repo, damostr)
	...
	def test_instantiation_with_kwargs(self):
		...
		bamo = BaseModel(id="345", created_at=datetime_iso, updated_at=datetime_iso)
		self.assertEqual(bamo.id, "345")
		self.assertEqual(bamo.created_at, datetime)
		self.assertEqual(bamo.updated_at, datetime)
	...
	def test_instantiation_with_args_and_kwargs(self):
		datetime = datetime.now()
		datetime_iso = dt.isoformat()
		bamo = BaseModel("12", id="345", created_at=datetime_iso, updated_at=datetime_iso)
		self.assertEqual(bamo.id, "345")
		self.assertEqual(bamo.created_at, datetime)
		self.assertEqual(bamo.updated_at, datetime)
...
class TestBaseModel_save(unittest.TestCase):
	"""Unittests for testing saved method of the BaseModel class."""
